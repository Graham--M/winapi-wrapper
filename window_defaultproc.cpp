...
LRESULT CALLBACK WndProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	do {
		if ( pWindow == 0 || *pWindow != hWnd ) {
			if ( uMsg == WM_NCCREATE ) {
				...
				PASSLPARAM* pass = reinterpret_cast<PASSLPARAM*>( lpCreate->lpCreateParams );
				if ( pass == 0 )
					break;
	...
	if ( b_def ) return DefWindowProc( hWnd, uMsg, wParam, lParam );
	switch ( uMsg ) {
	case WM_NCDESTROY:
...
WindowClass::WindowClass( UINT style, int cbClsExtra, int cbWndExtra, HICON hIcon, HCURSOR hCursor, HBRUSH hbrBackground, LPCTSTR lpszMenuName, HINSTANCE hInstance )
{
	Register( WndProc, style, cbClsExtra, cbWndExtra, hIcon, hCursor, hbrBackground, lpszMenuName, hInstance );
}

WindowClass::WindowClass( HICON hIcon, HBRUSH hbrBackground, LPCTSTR lpszMenuName, UINT style, HCURSOR hCursor, HINSTANCE hInstance )
{
	Register( WndProc, style, 0, 0, hIcon, hCursor, hbrBackground, lpszMenuName, hInstance );
}
